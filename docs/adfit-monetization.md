...
The `*.afterSummary` placements read the new `AFTER_SUMMARY` keys first. If a new key is empty, the client build falls back to the matching legacy `AFTER_RESULT` key so existing deployed AdFit unit IDs keep working during migration.
